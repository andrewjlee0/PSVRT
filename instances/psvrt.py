...
class psvrt(feeders.Feeder):
    ...
    def initialize_vars(self,
        ...
        if self.grid_size is not None:
            self.num_grid = 1 + (self.box_extent[0] - (self.item_size[0] - 1) - 1)/self.grid_size
        ...
        for pp in range(self.num_items):
            ...
            while position_flag == 0:
                ...
                if self.grid_size is None:
                    ...
                    new_position =  [np.random.randint(low=0, high=self.num_grid),
                                     np.random.randint(low=0, high=self.num_grid)]
                    new_position[0] *= self.grid_size
                    new_position[1] *= self.grid_size
                for old_position in positions_list:
                    position_viability = check_position_viability(new_position, old_position, SR_label, SR_portion, self.item_size, SR_type)
